...
void number_picker_draw(NumberPicker* instance)
{
  ...
  {
    LineRender line = render_line_init(
    ...
    LabelRender label = render_label_init(
      instance->Position.X + (DIV_WIDTH*i) + Font11x18.FontWidth/2 + 1,
...
static void number_picker_drawseg(NumberPicker* instance, int segment)
{
  ...
  render_draw_rectangle(
    render_rect_init(instance->Position.X + (segment*DIV_WIDTH) + DIV_STROKE,
    instance->Position.Y,
  ...
  LabelRender label = render_label_init(
    instance->Position.X + (DIV_WIDTH*segment) + Font11x18.FontWidth/2 + 1,
    instance->Position.Y + Font11x18.FontHeight/2 - 1,
    (char*)&str,
    Font11x18,
    selected ? ILI9341_COLOR_ORANGE : ILI9341_COLOR_BLUE,
